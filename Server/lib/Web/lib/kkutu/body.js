...
function requestRoomInfo(id) {
	...
	$data._roominfo = id;
	$("#RoomInfoDiag .dialog-title").text(id + L['sRoomInfo']);
	$("#ri-title").text((o.password ? "🔒 " : "") + o.title);
	...
	$(".profile-head").empty().append($pi = $("<div>").addClass("moremi profile-moremi"))
		.append($("<div>").addClass("profile-head-item")
			...
			.append($("<div>").addClass("profile-title ellipse").text(o.profile.title || o.profile.name)
				.append($("<label>").addClass("profile-tag").text(" #" + o.id.toString().substr(0, 5)))
			)
		)
		.append($("<div>").addClass("profile-head-item")
...
function getCharObject(char, subChar, wordLength) {
	var $res = $("<span>").text(char + (subChar ? ("(" + subChar + ")") : ""));
	if (wordLength) $res.append($("<label>").addClass('jjo-display-word-length').text("(" + wordLength + ")"));
	return $res;
}
...
function chat(profile, msg, from, timestamp) {
	...
	if (link = msg.match(/https?:\/\/[\w\.\?\/&#%=-_\+]+/g)) {
		var escapedMsg = $msg.text();
		link.forEach(function (item) {
			var safeLink = item.replace(/"/g, '&quot;').replace(/'/g, '&#39;');
			escapedMsg = escapedMsg.replace(item, "<a href='#' style='color: #2222FF;' onclick='if(confirm(\"" + L['linkWarning'] + "\")) window.open(\"" + safeLink + "\");'>" + item + "</a>");
		});
		$msg.html(escapedMsg);
	}
	if (from) {
		if (from !== true) $data._recentFrom = from;
		var $whisperTag = $("<label>").css({ 'color': "#7777FF", 'font-weight': "bold" }).text("<" + L['whisper'] + ">");
		$msg.prepend($whisperTag);
	}
