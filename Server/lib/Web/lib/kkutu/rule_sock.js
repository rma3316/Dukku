...
$lib.Sock.turnEnd = function (id, data) {
	var $sc = $("<div>").addClass("deltaScore").text("+" + data.score);
	var $uc = $("#game-user-" + id);
	var key;
	var i, j, l;
...
$lib.Sock.drawMaps = function () {
	...
	function $word(text) {
		...
		for (i = 0; i < len; i++) {
			$R.append($c = $("<div>").addClass("bb-char").text(text.charAt(i)));
...
$lib.Sock.drawDisplay = function () {
	...
	va.forEach(function (item, index) {
		$a.append($c = $("<div>").addClass("sock-char sock-" + item).css({ width: size, height: size }).text(item));
		if ($data._va[index] && $data._va[index] != item) {
			$c.text($data._va[index]).addClass("sock-picked").animate({ 'opacity': 0 }, 500);
		}
	});
	$data._va = va;
