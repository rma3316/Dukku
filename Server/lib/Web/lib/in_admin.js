...
(function () {
	...
	$(document).ready(function () {
		...
		$("#gamsi-go").on('click', function (e) {
			...
			for (i in list) {
				$data.append($("<tr>").attr('id', "gamsi-" + list[i]).append($("<td>").text("(" + list[i] + ") 감시 시작")));
				onGamsi();
			}
			i = 0;
			...
			function onGamsi() {
				...
				$.get("/gwalli/gamsi?id=" + cid, function (res) {
					if (!res) return $obj.text("(없는 사용자)" + cid);
					$obj.empty();
					[res._id, res.title || "-", res.server].forEach(function (v, idx) {
						var $td = $("<td>");
						if (idx == 2) {
							$td.append($("<a>").attr({ 'target': "_blank", 'href': "/?server=" + v }).text(v));
						} else {
							$td.text(v);
						}
						$obj.append($td);
		...
		$("#kpw-query").on('click', function (e) {
			...
			function getTable(prev, data) {
				...
				data.forEach(function (item, index) {
					...
					item.delta = pd ? ("(+" + pd + ")") : '-';
					$R.append($("<tr>")
						.append($("<td style='width: 20px; text-align: center; background-color: #EEEEEE;'>").text(rank + 1))
						.append($("<td style='width: 200px;'>").text(item._id))
